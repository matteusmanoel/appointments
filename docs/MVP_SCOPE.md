# MVP Scope — BarberFlow On-Prem

## Modelo de instalação

- **Single-tenant por cliente**: cada instalação on-prem atende **uma barbearia**.
- Uma instância = um `barbershop_id` (fixo ou único no banco).
- Multi-tenant no schema é mantido para evolução futura (SaaS); no MVP o painel e as tools operam sobre a barbearia da instalação.
...
| **Barbearia** | Dados básicos (nome, telefone, email, endereço) | Sim |
...
| **Configurações** | Seções de configuração (dados da barbearia, horários, etc.) | Sim (navegação + dados da barbearia) |
...
- Múltiplas barbearias na mesma instalação (multi-tenant ativo).
...
2. Painel admin com login e CRUD de barbearia, barbeiros, serviços, clientes e agenda.
